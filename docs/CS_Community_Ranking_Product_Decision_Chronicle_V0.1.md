...
**Owner 关于页卡片收束** 2026-08-30（V0.1.41；用灵感与数据替换候选池卡片；作者与灵感来源改为与「本项目已开源」同类的标签行，不再当标题）

**Owner 往期赛事与 BLAST S2** 2026-08-30（V0.1.42；EWC 归档到「往期赛事」；当期改为 BLAST Open Porto / BLAST S2，规则与 EWC 相同）

**Owner 榜单总计票数** 2026-08-30（V0.1.43；社区榜「入榜选手」旁显示计入总榜的有效票数）

**Owner 关于页版本脚注** 2026-08-31（V0.1.44；关于页 01–03 下显示当前版本号）
...
2026-08-30 Owner 去掉关于页「候选池」长文卡片，改由「灵感与数据」与「作者与开源」并排；「作者」和「灵感来源」不再当卡片标题，改成与「本项目已开源 / 数据支持」相同的标签加筹码。同日把 EWC 归档到新导航「往期赛事」（榜单式列表，点进后仍是原来的详细 MVP 表）；当期改为 BLAST Open Porto 2026（导航「当期赛事 - BLAST S2」），HLTV event 8249，规则与 EWC 相同（每日 +1、Top 10 加掉出保留、成绩来自官方奖金表）。同日社区榜在「入榜选手」旁增加「总计票数」，只计当期 Edition 计入总榜的有效票（`VALID` 且非 Skip），不含赛事 MVP、限流和可疑票。

2026-08-31 Owner 要求关于页用脚注标出版本号（当前 `V0.1.44`）。同时修好 GitHub `quality`：未过 Prettier 的换行会使 push CI 在约一分钟内失败。
...
**Owner 2026-08-20 有限解冻。** 常驻榜已上线后，当期赛事改用独立列表：访客每天给一名选手 +1，不再使用 pair 真随机和 +1/−1。当时硬编码 EWC，Top 10 来自官方 HLTV 赛事 Rating 3.0。这验证的是「每日一票」而不是原冻结问题本身。原冻结理由仍适用于把 Event MVP 做成另一套 Ballot Engine。

**Owner 2026-08-30。** EWC 归档到「往期赛事」；当期改为 BLAST Open Porto 2026（导航「当期赛事 - BLAST S2」）。同一套每日 +1、Top 10 加掉出保留、成绩来自官方奖金表。库中只允许一场 `ACTIVE`。

**Owner 2026-08-22。** HLTV Top 10 有人新进则加入名单；掉出 Top 10 的人留在投票表并保留已得票。成绩列取官方 HLTV 赛事奖金表上的战队名次（小组赛/十六强/八强/四强，决赛名次在奖金表写出队伍后再记冠军/亚军），并作为 Rating 与 Maps 之间的并列打破键。

**Owner 2026-08-24。** 每次 Event 同步必须同时回采选手 Rating/Maps/名次 **和** 奖金表战队成绩。只更新选手数据、不改写 `teamStanding` 会让页面继续显示过期的四强。奖金表已写出 1st–4th 后，把 `SEMIFINAL` 提升为冠军/亚军/季军/殿军。
...
|------------------------------|---------------------------------------------|---------------------------------------------------------------------------|
| **Event MVP（pair +1/−1）** | 短期窗口下真随机和 +1/-1 是否稳定尚未验证。 | 若要把 Event MVP 做成另一套 Ballot Engine。当前 `/current-event` 是每日 +1 列表，见 `docs/EVENT_MVP.md`。 |
