...
`/current-event` (live BLAST Open Porto / BLAST S2 as of 2026-08-30) plus `/past-events` for
archived contests (EWC). Daily visitor `+1` (not pair `+1/−1`), Maps and 成绩 columns, documented
in `docs/EVENT_MVP.md`. Still not included:
...
- When the live `/stats/players/` adapter is Cloudflare-blocked, a local Playwright capture of
  ...
  Past-3-month Rating 3.0, maps, Firepower, Majors won, Total MVPs, Top 20 overview years, and the
  profile flag (ISO-2). Do not hand-copy 70 rows, do not infer HLTV Rating from Liquipedia/BO3, do
  not enable the live adapter to bypass the block, and never run that capture in CI. Career Rating,
  ADR, and Round Swing stay explicitly missing until an official stats page can be parsed. Top 20
  years are stored as `top20_rank` CAREER snapshots on the existing `player_stat_snapshot` table;
  nationality updates `player.country_code` in the same audited import.
